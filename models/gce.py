...
class Model(nn.Module):
            ...
            for i, e in enumerate(batch):
                for s, v in e.turn_label:
                    try:
                        ...
                        print('ERROR : slots value error')
                        print(labels[s])
                        print(labels[s][i])
                        print(self.ontology.values[s])
                        print(self.ontology.values[s].index(v))
    ...
    def run_test(self, test, args):
        test.record_preds(
            preds=self.run_pred(test, self.args),
            to_file=os.path.join(self.args.dout, 'test.pred.json'),
        )
        print('saved test pred')
        predictions = self.run_pred(test, args)
        print(test.evaluate_preds(predictions))
